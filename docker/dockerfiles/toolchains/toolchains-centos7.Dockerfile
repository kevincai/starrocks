...
RUN yum install -y gcc gcc-c++ make automake curl wget gzip gunzip zip bzip2 file texinfo zlib-devel zlib-static && yum clean metadata
...
RUN yum install -y bison gettext flex dejagnu glibc-static sharutils libstdc++-static rpm-build && yum clean metadata
...
RUN /workspace/build_gcc.sh \
    ...
    --bootstrap-gcc-path ${GCC_10_INSTALL_HOME}/usr
