...
CONFIG_OPTS="-v --disable-multilib --enable-languages=c,c++ --with-gcc-major-version-only \
    ...
    --enable-plugin --enable-default-pie --with-system-zlib --enable-libphobos-checking=release \
    --with-target-system-zlib=auto --enable-multiarch \
    --disable-werror --enable-cet --with-tune=generic --enable-offload-defaulted \
...
make DESTDIR=${GCC_INSTALL_HOME} install
# Strip binaries to reduce size, ensuring paths exist first
if [ -d "${GCC_INSTALL_HOME}/usr/bin" ]; then
    strip ${GCC_INSTALL_HOME}/usr/bin/* 2>/dev/null || true
fi
if [ -d "${GCC_INSTALL_HOME}/usr/libexec/gcc" ]; then
    strip ${GCC_INSTALL_HOME}/usr/libexec/gcc/*/*/{cc1,cc1plus,collect2,lto1} 2>/dev/null || true
fi
