...
StarRocks runs its Java-based components (the FE, and the Java UDF, connector, and Java Extensions features on the BE and CN) on a JDK. Install the JDK version required by your StarRocks version before deployment, and then point `JAVA_HOME` to it.
