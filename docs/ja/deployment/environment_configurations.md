...
StarRocks の Java コンポーネント（FE、および BE と CN 上の Java UDF、コネクター、Java Extensions 機能）は JDK 上で動作します。デプロイ前に、お使いの StarRocks バージョンが要求する JDK バージョンをインストールし、その JDK を `JAVA_HOME` に指定してください。
